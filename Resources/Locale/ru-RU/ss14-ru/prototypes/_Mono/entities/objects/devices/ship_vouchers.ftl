# Прочие
ent-BaseShipLPC = ship LPC
    .desc = Содержит необходимые данные для консоли верфи для производства определённого проекта.
ent-ShipVoucherFrontierIronMan = ваучер на 3-22 «Бег»
    .desc = Позволяет бесплатный приобрести одино судно «Бег» на верфи. Уничтожается при продаже.
ent-ShipVoucherHound = ваучер на «Гончая»
    .desc = Небольшая карта, содержащая данные для получения дрона класса «Гончая» из резервов авианосца «Опустошитель».
ent-ShipVoucherRavager =  ваучер на «Опустошитель»
    .desc = Позволяет бесплатный приобрести один авеаносец «Опустошитель» из резервов Центрального Аванпоста.
ent-ShipVoucherRook = ваучер на «Ладья»
    .desc = Небольшая карта, содержащая данные для получения дрона класса «Ладья» из резервов авианосца «Опустошитель».
ent-ShipVoucherTaser = ваучер на «Тазер»
    .desc = Небольшая карта, содержащая данные для получения дрона класса «Тазер» из резервов авианосца «Опустошитель».


# ТСФ
ent-ShipVoucherFlyssa = ваучер на ТСФ-СКР «Флайсса-К»
    .desc = Небольшая карта, содержащая данные для получения эсминца класса «Флайсса-К» из резервов флагманского корабля ТСФ.
ent-ShipVoucherAldebaran = ваучер на TSF-PIN «Альдебаран»
    .desc = Небольшая карта, содержащая данные для получения фрегата класса «Альдебаран» из резервов флагманского корабля ТСФ.
ent-ShipVoucherAltair = ваучер на TSF-PIN «Альтаир»
    .desc = Небольшая карта, содержащая данные для получения фрегата класса «Альтаир» из резервов флагманского корабля ТСФ.
ent-ShipVoucherAndromeda = ваучер на ТСФ-CRSW «Андромеда»
    .desc = Небольшая карта, содержащая данные для получения крейсера класса «Андромеда» из резервов флагманского корабля ТСФ.
ent-ShipVoucherDagger = ваучер на ТСФ-СКР «Даггер»
    .desc = Небольшая карта, содержащая данные для получения рельсотронного эсминца класса «Даггер» из резервов флагманского корабля ТСФ.
ent-ShipVoucherMercury = ваучер на TSF-SKR «Меркурий»
    .desc = Небольшая карта, содержащая данные для получения корвета класса «Меркурий» из резервов флагманского корабля ТСФ.
ent-ShipVoucherSentinel = ваучер на ТСФ-СКР «Страж»
    .desc = Небольшая карта, содержащая данные для получения корабля класса «Страж» из резервов флагманского корабля ТСФ.
ent-ShipVoucherSpica = ваучер на TSF-PIN «Спика»
    .desc = Небольшая карта, содержащая данные для получения корабля класса «Спика» из резервов флагманского корабля ТСФ.
ent-ShipVoucherTarantula = ваучер на ТСФ-SVE «Тарантул»
    .desc = Небольшая карта, содержащая данные для получения корабля класса «Тарантул» из резервов флагманского корабля ТСФ.
ent-ShipVoucherTsfT0 = корабельный ваучер ТСФ уровня T0
    .desc = Позволяет бесплатно приобрести один малый корабль TSFMC. Уничтожается при продаже.
ent-ShipVoucherTsfT1Random = случайный ваучер ТСФ
    .desc = { ent-ShipVoucherTsfT0.desc }
    .suffix = Т1
ent-ShipVoucherTsfT2Random = случайный ваучер ТСФ
    .desc = { ent-ShipVoucherTsfT0.desc }
    .suffix = Т2
ent-ShipVoucherBalor = ваучер «Балор»
    .desc = Небольшая карта, содержащая данные для получения любого дрона из резервов верфи «Балор».
ent-ShipVoucherTsfT1 =  корабельный ваучер ТСФ уровня T1
    .desc = Позволяет бесплатно приобрести один средний корабль TSFMC. Уничтожается при продаже.
ent-ShipVoucherTsfT2 = корабельный ваучер ТСФ уровня T2
    .desc = Позволяет бесплатно приобрести один большой корабль TSFMC. Уничтожается при продаже.
ent-ShipVoucherPolaris =  ваучер на TSF-PIN «Полярис»
    .desc = Небольшая карта, содержащая данные для получения корабля класса «Полярис» из резервов флагманского корабля ТСФ.
ent-ShipVoucherFujian = ваучер на TSF-PIN «Fujian»
    .desc = Небольшая карта, содержащая данные для получения корабля класса «Fujian» из резервов флагманского корабля ТСФ.
